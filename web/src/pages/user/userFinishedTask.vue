        ...
        width="250"
          ...
          <template slot-scope="scope" >
            <el-button @click="deletetask(scope.row)"
                       type="primary" style="margin-left:20%" >删除</el-button>
          </template>
        </el-table-column>
      </el-table>
                ...
                switch (content[i].type) {
                  ...
                  case 'illegal':
                    ...
                    if (inputDataJson.illegalAcitivitycategory != '') {
                      temp1['非法活动名称'] = inputDataJson.illegalAcitivitycategory
                    }
                    if (inputDataJson.address != '') {
                      temp1['查询的地址'] = inputDataJson.address
                    }
        ...
        this.$axios.post('http://10.176.34.167:8000/api/getTaskResult', {
          ...
          switch (content.QueryItem) {
            case '行业分析':
              ...
              router.push('/singleChainAnalysis/Btc/BtcDataAnalysis/communityGraph')
              break
            case '非法活动':
              this.$cookies.set('task_id', content.id)
            ...
            case '资金追踪':
              ...
              this.$cookies.set('task_id', content.id)
              // this.$cookies.set('tracking_form', JSON.parse(content.QueryData))
              router.push('/singleChainAnalysis/Btc/BtcDataAnalysis/TrackingModelShowing')
              break
            case '地址关联分析':
              this.$cookies.set('task_id', content.id)
