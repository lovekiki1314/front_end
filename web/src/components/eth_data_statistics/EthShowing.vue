  ...
  methods: {
    ...
    get: function (url, root_api = 'stats') {
      ...
      if (Object.keys(this[url]).length === 0) {
        ...
        var full_url = this.HOST + '/statistics/everyday/stats/eth/' + url
        ...
        this.$axios
          ...
          .then(function (res) {
            ...
            switch (url) {
              case 'blocks': {
                  arr[3] =  res.data['total_blocks']
                  arr[2] = res.data['total_block_size'] 
                  arr[1] = res.data['avg_block_size']
                  arr[0] =res.data['avg_block_txs']
                //1
                _this[url].title.text = '以太坊区块信息统计曲线'
                // _this[url].yAxis.name = '地址个数';
                svg_obj_key[3] = '每日区块总数（个）'
                svg_obj_key[2] = '每日区块总大小（bytes）'
                svg_obj_key[1] = '平均区块大小（bytes）'
                svg_obj_key[0] = '平均交易（次）'
                break
              }
              case 'transactions': {
                arr[0]= res.data['total_txs']
                _this[url].title.text = '以太坊区块交易信息统计'
                // _this[url].yAxis.name = '地址个数';
                svg_obj_key[0] = '每日交易总数'
                 break
              }
               case 'gasanduncles':{
                 arr[0]=res.data['avg_gas']
                 arr[1]=res.data['avg_uncles']
                _this[url].title.text = '以太坊区块gas及叔块信息统计曲线'
                // _this[url].yAxis.name = '地址个数';
                svg_obj_key[0] = '平均每日gas使用量'
                svg_obj_key[1] = '平均每日区块叔块数'
                break
               }
               case 'price':{
                 arr[4]=res.data['open']
                ...
                _this[url].title.text = '以太坊历史行情统计曲线'
                // _this[url].yAxis.name = '地址个数';
    ...
    whether_show_get_before: function () {
      ...
      if (this.miner_search_flag == '1') {
        ...
        if (this.click_flag == '0') {
          this.form.start_date = this.customFormatter(new Date(2020, 10, 1))
          this.form.end_date = this.customFormatter(new Date(2020,11, 31))
