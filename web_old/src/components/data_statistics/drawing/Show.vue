    ...
    <div v-if="this.click_flag!=''">
      ...
      <div v-if="this.show_flag!='p'"
        ...
        <b-button variant="primary"
                  id="srh_btn"
                  @click="validate_choose_date">查询</b-button>
      </div>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    get: function (url, root_api = 'stats') {
      ...
      if (Object.keys(this[url]).length === 0) {
        ...
        var top_url = this.HOST + '/statistics/' + root_api + '/address/top_balance'      //余额top50的接口URL
        var form = {
          user_id : this.$store.state.id
        }
        this.$axios
        ...
        var full_url1 = this.HOST + '/statistics/' + root_api + '/' + url
        var full_url = 'http://10.176.34.161:8000/api/basicquery/block/eth/123/'
    ...
    srh_data: function () {
      ...
      //请求和处理的方法与普通折线图基本一致
      var full_url = this.HOST + '/statistics/stats/miner_fee'
      var jsonpara = this.form
      this.$axios
